...
def parse_gt_answer(ans_str):
    pattern = r'#### (.*)'
    ans = re.findall(pattern, ans_str)[-1]
    return ans
...
if __name__ == "__main__":
    ...
    for data_file in data_files:
        ...
        agent_answer = np.zeros((er, dr), dtype=float)

        for traj_id, traj in data.items():
            gt_answer[traj_id] = float(parse_gt_answer(traj['answer']))
            
            for debate_context in traj['states']:
                #agent_answer[traj_id][debate_context['round']] = debate_context['text_answer']
                try:
                    agent_answer[traj_id][debate_context['round']] = float(debate_context['majority_answer']) or np.nan
                except KeyError:
                    agent_answer[traj_id][debate_context['round']] = float(find_majority(debate_context['text_answer']) or np.nan )
