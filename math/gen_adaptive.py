...
Required JSON structure:
{
    ...
    "answer": "$\\boxed{?}$" // string, your final answer where the ? filled by a single numerical number
}
    ...
    for eval_round in tqdm(range(evaluation_round), total=evaluation_round, position=0, desc='Eval', leave=False, colour='#82b0d2', unit='traj'):
        ...
        for round in tqdm(range(debate_round), total=debate_round, position=1, desc='Debate', leave=False, colour='#8ecfc9', unit='round'):
            ...
            if (eval_round+1) % max(1,int(evaluation_round // 10)) == 0:
                pickle.dump(results,open("progress_data/{}/multi_gsm_results_er{}_agents{}_dr{}_ratio{}_range{}_{}.p".format(experiment_name, evaluation_round, num_agents, debate_round,0.0,args.question_range,eval_round),'wb'))
    pickle.dump(results,open("data/{}/multi_gsm_results_er{}_agents{}_dr{}_ratio{}_range{}.p".format(experiment_name, evaluation_round, num_agents, debate_round,0.0,args.question_range),'wb'))
