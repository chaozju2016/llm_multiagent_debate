...
if __name__ == "__main__":
    ...
    for data_file in data_files:
        ...
        for traj_id, traj in data.items():
            ...
            print(traj['question'], traj['answer'])
            print(gt_answer[traj_id])
            # break
            for debate_context in traj['states']:
                try:
                    agent_answer[traj_id][debate_context['round']] = debate_context['majority_answer'] or np.nan
                except KeyError:
                    agent_answer[traj_id][debate_context['round']] = find_majority(debate_context['text_answer'])
                print(debate_context['text_answer'], debate_context['majority_answer'] )
